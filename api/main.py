...
@app.post("/api/transactions/ai-suggest")
def ai_suggest_categories(body: dict):
    """
    Utilise Claude pour suggérer une catégorie sur une liste de transactions non catégorisées.
    Entrée : { transactions: [{id, label, amount, direction, date}, ...] }
    Sortie : { suggestions: [{id, suggested_category, confidence}, ...] }
    Ponctuel — ne modifie rien en DB, retourne juste les suggestions.
    """
    try:
        import anthropic as _anthropic
    except ImportError:
        return {"status": "error", "detail": "Package anthropic non installé"}

    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        return {"status": "error", "detail": "ANTHROPIC_API_KEY manquante"}
    ...
    cats = sb.table("category_mapping").select("pennylane_category_name,poste_budgetaire,pl_section").execute().data
    cat_list = sorted({r["pennylane_category_name"] for r in cats})

    # Construire le prompt
    tx_lines = "\n".join(
        f'- id={t["id"]} | {t["date"]} | {t["direction"]} | {float(t["amount"]):.2f}€ | "{t["label"]}"'
        for t in transactions
    )
    cat_lines = "\n".join(f"  - {c}" for c in cat_list)

    prompt = f"""Tu es un comptable expert en catégorisation de transactions bancaires pour une entreprise de formation en ligne (infopreneuriat).

Voici les catégories disponibles dans le plan comptable :
{cat_lines}

Pour chaque transaction ci-dessous, retourne la catégorie la plus adaptée parmi la liste ci-dessus.
Si aucune catégorie ne convient, retourne une chaîne vide "".

Règles importantes :
- "payment_paid_out" ou "payment_paid" dans le libellé = encaissement client (revenue)
- "gocardless_fee" ou "surcharge" = Frais transactions
- "Stripe Payout" ou "STRIPE TECHNOLOGY" sans "Charge:" = Virement interne Stripe
- "Qonto" = Frais bancaires ou Logiciel Structure
- "Automatic Taxes" = TVA décaissée
- "Charge:" + nom de formation = revenue correspondant à cette formation

Transactions :
{tx_lines}

Réponds UNIQUEMENT en JSON valide, format exact :
{{"suggestions": [{{"id": "...", "category": "...", "confidence": "high|medium|low"}}]}}"""
    ...
    if not match:
        return {"status": "error", "detail": "Réponse IA non parseable", "raw": raw[:200]}

    result = json.loads(match.group())
    return {"status": "ok", "suggestions": result.get("suggestions", []), "model": "claude-3-5-haiku"}


@app.post("/api/transactions/approve-all")
def approve_all_categories(body: dict):
    """
    Valide en batch toutes les suggestions fournies.
    Entrée : { items: [{id, label, date, category_name}, ...] }
    Applique approve_category sur chacun silencieusement.
    """
    ...
    for item in items:
        tx_id    = item.get("id")
        cat      = (item.get("category_name") or "").strip()
            ...
            if label:
                ...
                if existing:
                    ...
                    sb.table("label_category_rules").insert({
                        "pattern": label, "category_name": cat, "validated_count": 1
                    }).execute()

            if tx_date:
                dates_to_recompute.add(tx_date)
            validated += 1
        except Exception as e:
            errors.append({"id": tx_id, "error": str(e)})

    # Recalculer P&L pour les dates concernées
    try:
        import sync as _sync_mod
        for d in dates_to_recompute:
            d_obj = date.fromisoformat(d)
            _sync_mod.recompute_day(d_obj)
    except Exception as e:
        logging.warning(f"approve-all recompute: {e}")

    return {"status": "ok", "validated": validated, "errors": errors, "dates": list(dates_to_recompute)}
